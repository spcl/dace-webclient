(self["webpackChunk_spcl_sdfv"] = self["webpackChunk_spcl_sdfv"] || []).push([["renderer_dir_memory_button_js"],{

/***/ "./renderer_dir/memory_button.js":
/*!***************************************!*\
  !*** ./renderer_dir/memory_button.js ***!
  \***************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "MemoryButton": () => (/* binding */ MemoryButton)
/* harmony export */ });
/* harmony import */ var _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @babel/runtime/helpers/toConsumableArray */ "./node_modules/@babel/runtime/helpers/esm/toConsumableArray.js");
/* harmony import */ var _babel_runtime_helpers_classCallCheck__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @babel/runtime/helpers/classCallCheck */ "./node_modules/@babel/runtime/helpers/esm/classCallCheck.js");
/* harmony import */ var _babel_runtime_helpers_assertThisInitialized__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @babel/runtime/helpers/assertThisInitialized */ "./node_modules/@babel/runtime/helpers/esm/assertThisInitialized.js");
/* harmony import */ var _babel_runtime_helpers_inherits__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @babel/runtime/helpers/inherits */ "./node_modules/@babel/runtime/helpers/esm/inherits.js");
/* harmony import */ var _babel_runtime_helpers_possibleConstructorReturn__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! @babel/runtime/helpers/possibleConstructorReturn */ "./node_modules/@babel/runtime/helpers/esm/possibleConstructorReturn.js");
/* harmony import */ var _babel_runtime_helpers_getPrototypeOf__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! @babel/runtime/helpers/getPrototypeOf */ "./node_modules/@babel/runtime/helpers/esm/getPrototypeOf.js");
/* harmony import */ var _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./renderer_util.js */ "./renderer_dir/renderer_util.js");
/* harmony import */ var _datahelper_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./datahelper.js */ "./renderer_dir/datahelper.js");







function _createForOfIteratorHelper(o, allowArrayLike) { var it; if (typeof Symbol === "undefined" || o[Symbol.iterator] == null) { if (Array.isArray(o) || (it = _unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; var F = function F() {}; return { s: F, n: function n() { if (i >= o.length) return { done: true }; return { done: false, value: o[i++] }; }, e: function e(_e) { throw _e; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var normalCompletion = true, didErr = false, err; return { s: function s() { it = o[Symbol.iterator](); }, n: function n() { var step = it.next(); normalCompletion = step.done; return step; }, e: function e(_e2) { didErr = true; err = _e2; }, f: function f() { try { if (!normalCompletion && it["return"] != null) it["return"](); } finally { if (didErr) throw err; } } }; }

function _unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return _arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return _arrayLikeToArray(o, minLen); }

function _arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }

function _createSuper(Derived) { var hasNativeReflectConstruct = _isNativeReflectConstruct(); return function _createSuperInternal() { var Super = (0,_babel_runtime_helpers_getPrototypeOf__WEBPACK_IMPORTED_MODULE_5__.default)(Derived), result; if (hasNativeReflectConstruct) { var NewTarget = (0,_babel_runtime_helpers_getPrototypeOf__WEBPACK_IMPORTED_MODULE_5__.default)(this).constructor; result = Reflect.construct(Super, arguments, NewTarget); } else { result = Super.apply(this, arguments); } return (0,_babel_runtime_helpers_possibleConstructorReturn__WEBPACK_IMPORTED_MODULE_4__.default)(this, result); }; }

function _isNativeReflectConstruct() { if (typeof Reflect === "undefined" || !Reflect.construct) return false; if (Reflect.construct.sham) return false; if (typeof Proxy === "function") return true; try { Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {})); return true; } catch (e) { return false; } }

// Copyright 2019-2021 ETH Zurich and the DaCe authors. All rights reserved.



var MemoryButton = /*#__PURE__*/function (_Button) {
  (0,_babel_runtime_helpers_inherits__WEBPACK_IMPORTED_MODULE_3__.default)(MemoryButton, _Button);

  var _super = _createSuper(MemoryButton);

  function MemoryButton(ctx, all_mem_analyses, target_bw) {
    var _this;

    (0,_babel_runtime_helpers_classCallCheck__WEBPACK_IMPORTED_MODULE_1__.default)(this, MemoryButton);

    _this = _super.call(this, ctx);
    _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.assert("Valid bandwidth", target_bw > 0.0);
    _this.Memory_Target_Bandwidth = new Number(target_bw);
    _this._display_image = {
      "1": "data:image/png;base64,",
      "-1": "data:image/png;base64,"
    };
    var databinding = {};
    var judgements = all_mem_analyses.data.map(function (x) {
      return x.judgement;
    });
    var majority = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.majority(judgements);

    _this.setButtonImage(_this._display_image[majority]);

    _this.dataparams = [all_mem_analyses, target_bw];
    var repcount = all_mem_analyses.repcount;
    var layout = new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Layout(_this.button_subwindow);
    layout.setRect("Title", new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(0, 0), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(100, 10), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.RU_DataViewText());
    layout.setRect("PercentBandwidth", new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(70, 60), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(30, 20), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.RU_DataViewNumberBlock().setTitle("PercentBandwidth").setOptions({
      display_title: false,
      text_align: "left",
      draw_bar: ["left"],
      padding: {
        left: 10
      }
    }).setDataAnalysisFunction(function (d) {
      var x = d;
      var expected_bandwidth = x.data.map(function (x) {
        return x.data.expected_bandwidth;
      });
      var target_bw = x.data[0].data.Memory_Target_Bandwidth;
      var ret = undefined;
      if (toplevel_use_mean) ret = Math.round(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.mean(expected_bandwidth) * 100. / target_bw);else if (toplevel_use_median) ret = Math.round(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.median(expected_bandwidth) * 100. / target_bw);else _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.assert("Undefined mode", false);
      _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.assert("is number", ret instanceof Number || typeof ret == "number");
      return ret;
    }).setColorScaling(function (x) {
      return 100 - x;
    }));

    var transthis = (0,_babel_runtime_helpers_assertThisInitialized__WEBPACK_IMPORTED_MODULE_2__.default)(_this);

    _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.assert("Valid bandwidth", _this.Memory_Target_Bandwidth > 0.0);
    layout.setRect("Bandwidth", new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(0, 60), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(70, 20), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.RU_DataViewFormLayout().setTitle("Bandwidth").setDataAnalysisFunction(function (d) {
      var x = d;
      var expected_bandwidth = x.data.map(function (x) {
        return x.data.expected_bandwidth;
      });
      var datasize = x.data.map(function (x) {
        return x.data.datasize;
      });
      var input_datasize = x.data.map(function (x) {
        return x.data.input_datasize;
      });
      var bytes_from_l3 = x.data.map(function (x) {
        return x.data.bytes_from_l3;
      }).map(function (x) {
        return _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sum(x);
      });
      var bytes_from_mem = x.data.map(function (x) {
        return x.data.bytes_from_mem;
      }).map(function (x) {
        return _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sum(x);
      }); // Differentiate between global and local analysis here

      var repcount = all_mem_analyses.repcount;

      if (all_analyses_global) {
        var chunksize = datasize.length / repcount;
        datasize = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.createChunks(datasize, chunksize, _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sum);
        input_datasize = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.createChunks(input_datasize, chunksize, _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sum);
        datasize = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.createChunks(datasize, chunksize, _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sum);
        bytes_from_l3 = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.createChunks(bytes_from_l3, chunksize, _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sum);
        bytes_from_mem = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.createChunks(bytes_from_mem, chunksize, _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sum);
      }

      var val = {};

      if (toplevel_use_mean) {
        val = {
          title: "Bandwidth",
          value: _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.valueToSensibleString(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.mean(expected_bandwidth), "programmer", "B/c")
        };
      } else if (toplevel_use_median) {
        val = {
          title: "Bandwidth",
          value: _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.valueToSensibleString(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.median(expected_bandwidth), "programmer", "B/c")
        };
      } else _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.assert("Undefined mode", false);

      return {
        fontsize: 16,
        rows: [val, {
          title: "Target Bandwidth",
          value: _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.valueToSensibleString(transthis.Memory_Target_Bandwidth, "programmer", "B/c")
        }, {
          title: "Bytes processed (SA)",
          value: _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.valueToSensibleString(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.median(datasize), "programmer", "B")
        }, {
          title: "Bytes imported into scope (SA)",
          value: _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.valueToSensibleString(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.median(input_datasize), "programmer", "B")
        }, {
          title: "Bytes requested from L3",
          value: _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.valueToSensibleString(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.median(bytes_from_l3), "programmer", "B")
        }, {
          title: "Bytes requested from memory",
          value: _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.valueToSensibleString(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.median(bytes_from_mem), "programmer", "B")
        }],
        padding: {
          left: 10,
          right: 10,
          top: 0,
          bottom: 0
        },
        rawdata: d
      };
    }));
    layout.setRect("Graph", new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(0, 10), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Pos(70, 50), new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.RU_DataViewBarGraph({
      type: "horizontalBar",
      xAxes: [{
        type: 'linear',
        display: true,
        position: 'bottom',
        id: 'data-axis',
        gridLines: {
          display: false
        },
        ticks: {
          beginAtZero: true
        },
        scaleLabel: {
          labelString: "Misses",
          display: true
        }
      }, {
        type: 'linear',
        display: false,
        ticks: {
          max: 1,
          min: -1
        },
        id: 'corr-axis',
        position: 'top',
        gridLines: {
          display: false
        },
        scaleLabel: {
          labelString: "Correlation",
          display: true
        }
      }],
      yAxes: [{
        display: false,
        position: 'left',
        id: 'thread-axis',
        gridLines: {
          display: true
        },
        scaleLabel: {
          labelString: "Thread",
          display: true
        }
      }]
    }).setDataAnalysisFunction(function (x) {
      if (x == null) return;
      var l2misses = x.data.map(function (x) {
        return x.data.L2_TCM;
      });
      var l3misses = x.data.map(function (x) {
        return x.data.L3_TCM;
      });
      var tot_cyc = x.data.map(function (x) {
        return x.data.TOT_CYC;
      });
      var bw_mem = x.data.map(function (x) {
        return x.data.mem_bandwidth;
      });
      var bw_l3 = x.data.map(function (x) {
        return x.data.l3_bandwidth;
      });
      var colors = _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.RU_DataViewBarGraph.colorList().slice(0, l2misses[0].length + 1 + 2);
      var datasets = [];

      var thread_group_func = function thread_group_func(z1) {
        var thread_grouped_z1 = [];

        var _iterator = _createForOfIteratorHelper(l2misses[0]),
            _step;

        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            var _x = _step.value;
            thread_grouped_z1.push([]);
          }
        } catch (err) {
          _iterator.e(err);
        } finally {
          _iterator.f();
        }

        var _iterator2 = _createForOfIteratorHelper(z1),
            _step2;

        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            var run_tuple = _step2.value;
            var l2ms_per_thread = run_tuple[0];
            var cycs_per_thread = run_tuple[1];

            for (var _i = 0; _i < l2ms_per_thread.length; ++_i) {
              thread_grouped_z1[_i].push([l2ms_per_thread[_i], cycs_per_thread[_i]]);
            }
          }
        } catch (err) {
          _iterator2.e(err);
        } finally {
          _iterator2.f();
        }

        return thread_grouped_z1;
      };

      var z1 = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.zip(l2misses, tot_cyc);
      var thread_grouped_z1 = thread_group_func(z1);
      var l2corr = thread_grouped_z1.map(function (x) {
        return _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sample_corr(x[0], x[1]);
      });
      var thread_grouped_z2 = thread_group_func(_datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.zip(l3misses, tot_cyc));
      var l3corr = thread_grouped_z2.map(function (x) {
        return _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.sample_corr(x[0], x[1]);
      }); // Now we average the correlations

      var avg_l2_corr = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.mean(l2corr);
      var avg_l3_corr = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.mean(l3corr);
      var i = 0;

      for (var _i2 = 0, _arr = [l2misses, l3misses]; _i2 < _arr.length; _i2++) {
        var tcm = _arr[_i2];
        var labelstr = "[Unknown]";
        if (tcm == l2misses) labelstr = "L2_TCM";
        if (tcm == l3misses) labelstr = "L3_TCM";
        var tmp = 0;
        if (toplevel_use_mean) tmp = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.zip2d(tcm).map(function (x) {
          return _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.mean(x);
        });else if (toplevel_use_median) tmp = _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.zip2d(tcm).map(function (x) {
          return _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.MathHelper.median(x);
        });else _datahelper_js__WEBPACK_IMPORTED_MODULE_7__.ObjectHelper.assert("Undefined mode", false);
        datasets.push({
          label: labelstr,
          xAxisID: "data-axis",
          yAxisID: "thread-axis",
          data: tmp,
          backgroundColor: colors[i]
        });
        i++;
      }

      var rho_col1 = colors[i];
      var rho_col2 = colors[i + 1];

      if (display_memory_correlation) {
        datasets.push({
          label: "L2 corr.",
          xAxisID: "corr-axis",

          /* yAxisID: "corr-y-axis",*/
          data: l2corr,
          backgroundColor: colors[i],
          hidden: true
        });
        datasets.push({
          label: "L3 corr.",
          xAxisID: "corr-axis",

          /*yAxisID: "corr-y-axis",*/
          data: l3corr,
          backgroundColor: colors[i + 1],
          hidden: true
        });
      }

      var chartData = {
        labels: (0,_babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_0__.default)(Array(l2misses[0].length).keys()).map(function (x) {
          return "Thread " + x.toString();
        }),
        "datasets": datasets
      };
      return chartData;
    }).linkMouse(_this.button_subwindow).changeGraphOptions(function (x) {
      x.options.title.text = "Cache misses (mean over all runs)";
      x.options.title.display = false;
      x.options.legend = {
        position: 'top',
        display: true
      };
    }));
    databinding["PercentBandwidth"] = all_mem_analyses;
    databinding["Title"] = new _renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.DataBlock({
      fontsize: 32,
      text: "Memory performance",
      color: "black",
      align: "center"
    }, "Text");
    databinding['Graph'] = all_mem_analyses;
    databinding['Bandwidth'] = all_mem_analyses;
    layout.setDataBinding(databinding);

    _this.button_subwindow.setLayout(layout);

    _this.setOnEnterHover(function (p) {
      _this.color = "#FF0000";
      _this.button_subwindow_state = 'open';
    });

    _this.setOnLeaveHover(function (p) {
      _this.color = "orange";
      if (!_this.is_locked_open) _this.button_subwindow_state = 'collapsed';
    });

    _this.setOnClick(function (p, mb) {
      _this.is_locked_open = !_this.is_locked_open;
    });

    _this.setDefaultDblClick();

    return _this;
  }

  return MemoryButton;
}(_renderer_util_js__WEBPACK_IMPORTED_MODULE_6__.Button);



/***/ })

}]);
//# sourceMappingURL=renderer_dir_memory_button_js.js.map