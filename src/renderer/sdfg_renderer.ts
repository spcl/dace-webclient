...
import { GraphElement, GraphNode } from './graph/graph_element';
    ...
    public do_for_intersected_elements(
        ...
        function traverse_recursive(
            ...
            g?.nodes().forEach((state_id: string) => {
                const state: dagre.Node<GraphElement> = g.node(state_id);
                if (!state)
                    return;
    ...
    public for_all_elements(
        ...
        function traverse_recursive(g: DagreSDFG | null, sdfg_name: string) {
            g?.nodes().forEach(state_id => {
                const state: State = g.node(state_id) as State;
                if (!state)
                    return;
...
function relayout_state(
    ...
    sdfg_state.edges.forEach((edge: JsonSDFGEdge, id: number) => {
        ...
        //    const src_node: GraphNode = g.node(edge.src);
        //    let cindex = -1;
        //    for (let i = 0; i < src_node.outConnectors.length; i++) {
        //        if (
        //            src_node.outConnectors[i].data.name == edge.src_connector
        //        ) {
        //            cindex = i;
        //            break;
        //        }
        //    }
        //    if (cindex >= 0) {
        //        gedge.points[0].x = src_node.outConnectors[cindex].x;
        //        gedge.points[0].y = src_node.outConnectors[cindex].y;
        //        src_conn = src_node.outConnectors[cindex];
        //    }
        //}
        if (edge.dst_connector) {
            //const dst_node: GraphNode = g.node(edge.dst);
            //let cindex = -1;
            //for (let i = 0; i < dst_node.inConnectors.length; i++) {
            //    if (dst_node.inConnectors[i].data.name == edge.dst_connector) {
            //        cindex = i;
            //        break;
            //    }
            //}
            ...
            //        dst_node.inConnectors[cindex].x;
            //    gedge.points[gedge.points.length - 1].y =
            //        dst_node.inConnectors[cindex].y;
            //    dst_conn = dst_node.inConnectors[cindex];
            //}
        }
