...
export default class OrderGraph {
    ...
    private _groupEdgesAdded = false;
    ...
    public order(options: object = {}): number {
        ...
        const doOrder = (graph: OrderGraph): number => {
            ...
            const groupOrder = []; // current order of groups on this level (e. g. 2, 0, 1)
            const groupPositions = [];// inverse of groupOrder (e. g. 1, 2, 0)
            const order = []; // current order of nodes on this level (e. g. 2, 0, 1)
            ...
            const countCrossings = (testOrder: Array<number>, r: number, direction: 'UP' | 'DOWN', preventConflicts = false) => {
                if (preventConflicts) {
             ...
             */
            const reorder = (shuffle = false, shuffleNodes = false, startRank = 0, preventConflicts = false) => {
                Timer.start(['doLayout', 'orderRanks', 'doOrder', 'order', 'doOrder', 'reorder']);
                if (shuffle) {
                    _.forEach(ranks, (rank: OrderRank, r: number) => {
                        if (options['orderGroups']) {
                            const maxLevel = _.maxBy(rank.groups, group => group.shuffleHierarchy.length).shuffleHierarchy.length - 1;
                ...
                while (improveCounter > 0) {
                    ...
                    const firstRank = downward ? startRank + 1 : ranks.length - 2;
                    const lastRank = downward ? ranks.length - 1 : startRank;
            ...
            const getConflict = (type: 'HEAVYHEAVY' | 'HEAVYLIGHT', r: number, skipIfZero = false) => {
            ...
            const resolveConflicts = () => {
                ...
                const resolveConflict = (conflict) => {
                    ...
                    const crossingNorthNode = ranks[r - 1].groups[0].nodes[crossingNorthN];
                    const crossingSouthN = order[r][crossingSouthPos];
                    const crossingSouthNode = ranks[r].groups[0].nodes[crossingSouthN];
                    ...
                    const resolveY = () => {
                        ...
                        const removeNode = (node: OrderNode, permanent = false) => {
                            const r = node.rank;
                            const n = node.index;
    ...
    private _getPartialOrders(newOrder: Array<number>, order: Array<number>, positions: Array<number>, debug = false): Array<Array<number>> {
        const changes = [];
