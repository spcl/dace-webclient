...
export default class LayoutNode extends Node<LayoutGraph, LayoutEdge> {
    ...
    public isAccessNode = false;
    public isScopeNode = false;
    public hasScopedConnectors = false;
    public rank: number = null; // global rank (level) of the node
    public rankSpan = 1;
    public index = 0; // index of the node, when indexes is set, it should eventually be the max index
    ...
    constructor(size: Size = null, padding = 0, isVirtual = false, isBundle = false) {
        super();
        if (size !== null) {
    ...
    addConnector(type: 'IN' | 'OUT', name: string, temporary = false): void {
